...
import java.util.concurrent.TimeUnit;
    ...
    public class StockComponent {
         ...
         * @return -2:库存不足; -1:不限库存; 大于等于0:扣减库存以后的剩余库存
         */
        ...
        public long addStock(String key, Long expire, int num) {
            ...
            Assert.notNull(expire,"初始化库存失败，库存过时时间不能为null");
            RedisStockLock redisLock = new RedisStockLock(redisTemplate, key);
            try {
                if (redisLock.tryLock()) {
                    // 获取到锁后再次判断一下是否有key
         ...
         * @param key 库存key
         * @param num 扣减库存数量
         * @return 扣减以后剩余的库存【-3:库存未初始化; -2:库存不足; -1:不限库存; 大于等于0:扣减库存以后的剩余库存】
         */
