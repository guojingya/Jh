...
public class RedisStockLock {
    ...
    /**
     * seconds — 以秒为单位设置 key 的过时时间，等效于EXPIRE key seconds
     */
    ...
    /**
     * 使用默认的请求锁的超时时间，指定锁的过时时间
     *
     * @param redisTemplate
     * @param lockKey       锁的key（Redis的Key）
     * @param expireTime    锁的过时时间(单位：秒)
     */
     ...
     * 能够经过如下修改，让这个锁实现更健壮：
     * <p>
     * 不使用固定的字符串做为键的值，而是设置一个不可猜想（non-guessable）的长随机字符串，做为口令串（token）。
     * 不使用 DEL 命令来释放锁，而是发送一个 Lua 脚本，这个脚本只在客户端传入的值和键的口令串相匹配时，才对键进行删除。
     * 这两个改动能够防止持有过时锁的客户端误删现有锁的状况出现。
     */
     ...
     * 客户端执行以上的命令：
     * <p>
     * 若是服务器返回 OK ，那么这个客户端得到锁。
     * 若是服务器返回 NIL ，那么客户端获取锁失败，能够在稍后再重试。
     *
     * @param key     锁的Key
     * @param value   锁里面的值
